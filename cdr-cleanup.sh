...
# SETUP UTILITAS
# =======================
TEMP_ALL_FILES=$(mktemp /var/tmp/cleanup_all_XXXXXX)
TEMP_DELETE_LIST=$(mktemp /var/tmp/cleanup_delete_XXXXXX)
...
print_to_terminal() {
    ...
    echo "[$timestamp_full] [$level] $message" >> "$LOGFILE"
    ...
    if command -v logger >/dev/null 2>&1 && [[ -v INVOCATION_ID ]]; then
        ...
        logger -t "disk-cleanup" -p "user.$journal_priority" "$message"
    fi
}
...
rotate_log() {
    if [[ -f "$LOGFILE" ]] && [[ $(stat -c%s "$LOGFILE" 2>/dev/null || echo 0) -ge "$MAX_LOG_SIZE" ]]; then
...
show_help() {
    ...
    cat << EOF
Disk Cleanup Utility for RHEL 9
--------------------------------
...
OPTIONS:
    ...
    --threshold=N         Batas persen disk usage (Default: 90).
    ...
    --directory=PATH      Target direktori (Default: /home/cdrsbx).
    --min-files=N         Minimum file yang WAJIB disisakan per folder (Default: 30).
    --max-delete=N        Batas maksimum file yang dihapus per eksekusi (Default: 100).
    ...
    --backup              Backup file sebelum dihapus ke /backup/deleted_files.
    --debug               Tampilkan debug messages ke terminal (default: debug hanya ke log).
    --quiet               Matikan semua output ke terminal (tetap log ke file).
    --help                Tampilkan pesan bantuan ini.

EXAMPLES RHEL 9:
    # 1. Simulasi cleanup dengan threshold 85%
    ./$script_name --threshold=85

    # 2. Hapus file tua > 180 hari, exclude pattern tambahan
    ./$script_name --force --age-days=180 --exclude="*.log" --exclude="backup/*"

    # 3. Cleanup dengan backup, debug mode, dan batas delete
    ./$script_name --force --threshold=80 --backup --debug --max-delete=500

    # 4. Untuk XFS filesystem (default RHEL 9)
    ./$script_name --directory=/data --force --threshold=75
...
NOTES:
    • Debug messages selalu dicatat di log file
    • Debug messages ke terminal hanya dengan opsi --debug
    • Mode quiet: semua output hanya ke log file
...
generate_delete_list() {
    ...
    find_cmd+=" ! -path \"$LOGFILE\" ! -path \"$LOGFILE.old\""
    find_cmd+=" ! -path \"$LOCKFILE\""
    ...
    find_cmd+=" ! -path \"*/cleanup_all_*\" ! -path \"*/cleanup_delete_*\""
    find_cmd+=" ! -path \"/tmp/cleanup_*\" ! -path \"/var/tmp/cleanup_*\""
...
main() {
    # Banner
    echo "=== DISK CLEANUP UTILITY FOR RHEL 9 ==="
    echo "Dengan proteksi: Exclude hidden files & user default directories"
    
    # ============================================
    # START LOG
    # ============================================
    print_to_terminal "=========================================" "HEADER"
    print_to_terminal "🚀 DISK CLEANUP STARTED" "HEADER"
    ...
    # Log configurasi
    print_to_terminal "Configuration:" "INFO"
    ...
    print_to_terminal "  Backup: $([ "$BACKUP_ENABLED" -eq 1 ] && echo "YES ($BACKUP_DIR)" || echo "NO")" "INFO"
    ...
    print_to_terminal "Lock file created: $LOCKFILE" "DEBUG"
    ...
    print_to_terminal "=========================================" "HEADER"
    print_to_terminal "✅ DISK CLEANUP COMPLETED" "HEADER"
    ...
    # Log file info
    if [[ -f "$LOGFILE" ]]; then
        local log_size=$(stat -c%s "$LOGFILE" 2>/dev/null || echo "N/A")
        print_to_terminal "Log File: $LOGFILE (Size: $log_size bytes)" "INFO"
    fi
